...
void MainWindow::initWidget()
{
    ...
    // m_rectStableCount = 0;
    // m_maxRectStableFrames = 5; // 连续5帧稳定才确认矩形
...
//     // 更新PPT矩形区域
//     updatePPTRectangle(currentFrame);

//     double changeRatio = 0.0;
//     bool usePPTRect = m_pptRectValid && m_pptRect.area() > 0;

//     cv::Mat currentROI, prevROI;

//     if (usePPTRect) {
//         currentROI = currentFrame(m_pptRect);
//         prevROI = m_prevFrame(m_pptRect);
//     } else {
//         currentROI = currentFrame;
//         prevROI = m_prevFrame;
//     }
...
//     // 更新检测信息显示
//     QString detectionInfo = QString("检测信息: %1 变化率: %2% 优质匹配: %3 / %4 特征点: %5 - %6 阈值: %7")
//                                 .arg(usePPTRect ? "PPT区域" : "全图")
...
// 检测PPT播放区域的矩形
// cv::Rect MainWindow::detectPPTRectangle(const cv::Mat& frame)
// {
...
//         if (area < 10000) continue; // 过滤小面积轮廓
//         // 多边形逼近
...
// // 更新PPT矩形区域
// void MainWindow::updatePPTRectangle(const cv::Mat& frame)
// {
//     cv::Rect newRect = detectPPTRectangle(frame);

//     if (newRect.area() > 0) {
//         // 检查新矩形与旧矩形的相似度
//         if (m_pptRectValid) {
//             double overlap = (m_pptRect & newRect).area() / (double)(m_pptRect | newRect).area();

//             if (overlap > 0.7) { // 重叠度大于70%认为是稳定的
//                 m_rectStableCount++;
//                 if (m_rectStableCount >= m_maxRectStableFrames) {
//                     m_pptRect = newRect; // 更新为稳定的矩形
//                 }
//             } else {
//                 m_rectStableCount = 0;
//                 m_pptRect = newRect; // 立即更新
//             }
//         } else {
//             m_pptRect = newRect;
//             m_pptRectValid = true;
...
//     qDebug() << "PPT矩形区域:" << m_pptRect.x << m_pptRect.y << m_pptRect.width << "x" << m_pptRect.height
//              << "稳定性:" << m_rectStableCount << "/" << m_maxRectStableFrames;
// }

// // 在图像上绘制PPT矩形
// void MainWindow::drawPPTRectangle(cv::Mat& frame)
// {
//     if (m_pptRectValid && m_pptRect.area() > 0) {
//         cv::rectangle(frame, m_pptRect, cv::Scalar(0, 255, 0), 3);
//         cv::putText(frame, "PPT Region",
//                     cv::Point(m_pptRect.x, m_pptRect.y - 10),
